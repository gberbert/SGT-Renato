...
- **Novos utilitários:** `src/utils/efficiencyMetrics.js` (`buildEfficiencySeries`, `computeEfficiencySummary`) e componente `src/components/operacao/OperacaoEfficiencyChart.jsx` (recharts `ComposedChart`, lazy-loaded).
- **Fix (Filtros de Data):** Corrigido visual dos filtros "Data de Criação" e "Data de Resolução" que apareciam colados um no outro; adicionada borda/gap de separação.
- **Fix (Gráfico Criados x Resolvidos):** Adicionados rótulos numéricos no topo de cada barra, na mesma cor dos ticks dos eixos, para leitura rápida dos valores sem depender do tooltip.
- **Deploy:** Build de produção + deploy de `hosting` e `functions` no projeto `sgt-renato`.

## [0.1.223] - 2026-08-24
- **Feature (Radar Operação — Filtros de Data):** Adicionados filtros de intervalo de `createdAt` e `resolvedAt` no painel superior de filtros, com seletor de período fixo (Dia, Semana, Mês, Trimestre corrido, Quarter civil, Semestre, Ano, Personalizado) além de inputs de calendário — novo utilitário `src/utils/dateRangePresets.js` e componente `OperacaoDateRangeFilter.jsx`.
- **Feature (Radar Operação — Abas por Escopo):** Substituídos os tiles fixos por abas (`Geral`, `Problemas`, `Demandas`, `Incidentes`, `Solicitações`, `Catálogo`). A aba "Geral" mantém todos os totais/tiles; as demais abas mostram o total do escopo + o novo gráfico "Criados x Resolvidos". Os filtros (grupo/squad/status/datas) ficam acima das abas e se aplicam a todas elas de forma consistente.
- **Feature (Radar Operação — Gráfico Criados x Resolvidos):** Novo gráfico de barras verticais (recharts, lazy-loaded para não inflar o bundle principal) comparando `createdAt` vs `resolvedAt`, com seletor de agrupamento (Dia/Semana/Mês/Trimestre/Quarter/Ano) — novo utilitário `src/utils/timeSeriesBuckets.js` e componente `OperacaoEscopoTimelineChart.jsx`. Exibido apenas nas abas de escopo (não em "Geral").
- **Perf/Fix (Cloud Function `getOperacaoRadarBootstrap`):** Removidos os fallbacks de full-scan de `tickets_global` (paginação de 500 em 500 na própria function) que causavam timeouts/travamentos no boot do Radar quando `operacao_stats/summary` estava com `byStatus`/`radarByEscopo` vazios. Agora, quando os agregados não existem, usa-se 6 aggregation queries (`count()`) por escopo — baratas e rápidas — em vez de escanear a coleção inteira.
- **Deploy:** Build de produção + deploy de `hosting` e `functions` no projeto `sgt-renato`.
