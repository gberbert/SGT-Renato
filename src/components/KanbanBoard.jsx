...
const DEFAULT_DEMANDAS_COLUMNS = [
  { id: "col-aguardando-gestor-imediato", title: "Aguardando Aprovação Gestor Imediato", statusId: "col-aguardando-gestor-imediato" },
  { id: "col-escrita-requerimento", title: "Escrita de Requerimento", statusId: "col-escrita-requerimento" },
  { id: "col-validacao-comite", title: "Validação Comitê", statusId: "col-validacao-comite" },
  { id: "col-detalhamento-requisitos", title: "Detalhamento de Requisitos", statusId: "col-detalhamento-requisitos" },
  { id: "col-aguardando-pro-ti", title: "Aguardando Profissional de TI", statusId: "col-aguardando-pro-ti" },
  { id: "col-aguardando-demanda-projeto", title: "Aguardando Demanda/Projeto", statusId: "col-aguardando-demanda-projeto" },
  { id: "col-revisao-requisitos-projeto", title: "Revisão de Requisitos de Projeto", statusId: "col-revisao-requisitos-projeto" },
  { id: "col-analise-t-shirt", title: "Análise e T-Shirt", statusId: "col-analise-t-shirt" },
  { id: "col-planejamento", title: "Planejamento", statusId: "col-planejamento" },
  { id: "col-aprovacao-planejamento", title: "Aprovação de Planejamento", statusId: "col-aprovacao-planejamento" },
  { id: "col-execucao", title: "Execução", statusId: "col-execucao" },
  { id: "col-em-execucao", title: "Em Execução", statusId: "col-em-execucao" },
  { id: "col-aprovacao-qa", title: "Aprovação QA", statusId: "col-aprovacao-qa" },
  { id: "col-em-testes", title: "Em Testes", statusId: "col-em-testes" },
  { id: "col-em-homologacao", title: "Em homologação", statusId: "col-em-homologacao" },
  { id: "col-revisao-homologacao", title: "Revisão de homologação", statusId: "col-revisao-homologacao" },
  { id: "col-etapa-kt", title: "Etapa de KT", statusId: "col-etapa-kt" },
  { id: "col-aguardando-mudanca", title: "Aguardando Mudança", statusId: "col-aguardando-mudanca" },
  { id: "col-concluida", title: "Concluída", statusId: "col-concluida" },
  { id: "col-cancelada", title: "Cancelada", statusId: "col-cancelada" },
];

const DEFAULT_ATIVIDADES_COLUMNS = [
...
const KanbanBoard = ({ onCardClick, userRole, board = "demandas", setIsModalOpen }) => {
  ...
  useEffect(() => {
    const fallback = board === "demandas" ? DEFAULT_DEMANDAS_COLUMNS : DEFAULT_ATIVIDADES_COLUMNS;
  ...
  useEffect(() => {
    if (selectedProjectId) localStorage.setItem("lastSelectedProjectId", selectedProjectId);
  }, [selectedProjectId]);
    ...
    if (isOverTicket) {
      setTickets((prev) => {
        ...
        if (activeIndex !== -1 && overIndex !== -1 && prev[activeIndex].columnId !== prev[overIndex].columnId) {
          ...
          newTickets[activeIndex] = { ...newTickets[activeIndex], columnId: prev[overIndex].columnId };
    ...
    if (isOverColumn) {
      setTickets((prev) => {
        ...
        if (activeIndex !== -1 && prev[activeIndex].columnId !== overId) {
          const newTickets = [...prev];
          newTickets[activeIndex] = { ...newTickets[activeIndex], columnId: overId };
  ...
  const handleDragEnd = async (event) => {
    ...
    if (targetColumnId) {
      const activeTicketOriginal =
        originalSnapshot || active.data.current?.ticket || tickets.find((t) => t.id === activeId);
      ...
      if (activeTicketOriginal) {
        try {
          ...
          const originalColumnId = activeTicketOriginal.columnId;
          ...
          if (finalAssignee && activeTicketOriginal.assignee !== finalAssignee) {
            await updateTicket(
              activeId,
              { assignee: finalAssignee === "Sem responsável" ? "" : finalAssignee },
              userName
            );
          }
        } catch (e) {
  ...
  if (error) {
    return (
      <div className="view-content" style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", height: "100%", gap: "16px" }}>
        ...
        <p style={{ color: "var(--text-muted)", maxWidth: "600px", textAlign: "center" }}>{error}</p>
      </div>
    );
  }
  ...
  let filteredTickets =
    selectedProjectId === "all" ? tickets : tickets.filter((t) => t.projectId === selectedProjectId);
  ...
  const assignees = useSwimlanes ? [...new Set(filteredTickets.map((t) => t.assignee || "Sem responsável"))] : [null];
      ...
      for (const issue of results) {
        try {
          ...
          const ticketData = {
            ...
            priority: jiraData.priority?.toLowerCase().includes("alta") ? "high" : jiraData.priority?.toLowerCase().includes("crítica") ? "critical" : "medium",
      ...
      <div className="kanban-wrapper" style={{ height: "100%", display: "flex", flexDirection: "column" }}>
        <div className="kanban-header">
              ...
              <Select.Root value={selectedSquadId} onValueChange={setSelectedSquadId}>
                ...
                <Select.Content>
                  ...
                  {squads.filter((sq) => !isLeader || allowedSquadIds.includes(sq.id)).map((sq) => (
                    <Select.Item key={sq.id} value={sq.id}>
                      {sq.name}
                    </Select.Item>
                  ))}
                </Select.Content>
          ...
          <Flex gap="2" className="kanban-actions" align="center">
            ...
            {viewMode === "kanban" && (
              <Button variant={useSwimlanes ? "solid" : "soft"} onClick={() => setUseSwimlanes(!useSwimlanes)} className="kanban-btn">
                <LayoutList size={16} />
            ...
            <Button variant={viewMode === "kanban" ? "solid" : "soft"} onClick={() => setViewMode("kanban")} className="kanban-btn">
              <LayoutGrid size={16} /> Kanban
            </Button>

            <Button variant={viewMode === "list" ? "solid" : "soft"} onClick={() => setViewMode("list")} className="kanban-btn">
              <List size={16} /> Lista
            </Button>
            ...
            <div style={{ paddingRight: "16px" }}>
              {columns.map((col) => {
                const colTickets = filteredTickets.filter(
                  (t) => t.columnId === col.statusId && (board === "atividades" || !t.parentId)
                );
                if (colTickets.length === 0) return null;

                return (
                  <Card key={col.id} mb="4" size="3">
                    <Text as="h3" size="4" weight="bold" mb="3">
                      {col.title} <Badge ml="2" radius="full">{colTickets.length}</Badge>
                    </Text>
