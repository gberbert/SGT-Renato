...
const TicketDetailsModal = ({ isOpen, onClose, ticket, userRole }) => {
  ...
  useEffect(() => {
    if (ticket) {
      ...
      setSquadIds(ticket.squadIds || (ticket.squadId ? [ticket.squadId] : []));
  ...
  const handleUpdateField = async (field, value) => {
    ...
    try {
      ...
      if (field === 'squadIds' && (!ticket.board || ticket.board === 'demandas')) {
        try {
          ...
          for (const d of childSnap.docs) {
            await updateTicket(d.id, { squadIds: value }, userName);
          }
        } catch (childErr) {
  ...
  const handleUpdateJiraDate = (field, value) => {
    const currentFlow = ticket.jiraDatesFlow || {};
    const updatedFlow = { ...currentFlow, [field]: value };
    // update local object immediately so UI doesn't bounce if not re-rendered fast enough
    ticket.jiraDatesFlow = updatedFlow;
    handleUpdateField('jiraDatesFlow', updatedFlow);
  };
            ...
            <Tabs.Content value="details" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
              <Box style={{ height: '100%' }}>
                ...
                <Flex direction="column" gap="4">
                    ...
                    {ticket.isAutoGenerated && (
                      <Card variant="surface" style={{ background: 'var(--amber-3)', border: '1px solid var(--amber-6)', marginBottom: '16px' }}>
                        ...
                        <Text size="2" color="amber" mt="1">
                          Edições estruturais devem ser feitas com cuidado. Essa atividade foi gerada automaticamente.
                        </Text>
                      </Card>
                    )}
                  ...
                  {(!ticket.board || ticket.board === 'demandas') && (
                    <Card variant="surface" style={{ background: 'var(--indigo-2)', border: '1px solid var(--indigo-5)', marginBottom: '16px' }}>
                      ...
                      <Grid columns="3" gap="3">
                        {[
                          ...
                          <Box key={field.key}>
                            ...
                            <TextField.Root 
                              type="date"
                              value={ticket.jiraDatesFlow?.[field.key] || ''}
                              onChange={(e) => handleUpdateJiraDate(field.key, e.target.value)}
                            />
                          </Box>
                        ))}
                      </Grid>
                  ...
                  <Grid columns="2" gap="4">
                    {squads.length > 0 && (
                      <Box>
                        <Text as="div" size="2" weight="bold" mb="1" color="gray">Squads</Text>
                        <div style={{ border: '1px solid var(--gray-5)', borderRadius: '4px', padding: '8px', maxHeight: '100px', overflowY: 'auto', opacity: ticket.isAutoGenerated ? 0.6 : 1, pointerEvents: ticket.isAutoGenerated ? 'none' : 'auto' }}>
                            ...
                            {squads.map(s => (
                              <label key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <input 
                                  ...
                                  onChange={(e) => {
                                     let newIds = squadIds || [];
                                     if (e.target.checked) newIds = [...newIds, s.id];
                                     else newIds = newIds.filter(id => id !== s.id);
                                     setSquadIds(newIds);
                                     handleUpdateField('squadIds', newIds);
                                  }}
                                />
